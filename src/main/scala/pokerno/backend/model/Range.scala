...
object Range {
  case class Disabled(amount: Decimal) extends Error("amount=%.2f".format(amount))
  case class GreaterThanMax(amount: Decimal, max: Decimal) extends Error("amount=%.2f max=%.2f".format(amount, max))
  case class LessThanMin(amount: Decimal, min: Decimal) extends Error("amount=%2.f min=%.2f".format(amount, min))
