// Generated by http://code.google.com/p/protostuff/ ... DO NOT EDIT!
// Generated from rpc.proto

package de.pokerno.protocol.rpc;

import java.io.IOException;

import com.dyuproject.protostuff.Input;
import com.dyuproject.protostuff.Output;
import com.dyuproject.protostuff.Schema;
import com.dyuproject.protostuff.UninitializedMessageException;

public class CreateRoomSchema
       implements Schema<CreateRoom> {


    static final CreateRoom DEFAULT_INSTANCE = new CreateRoom();
    static final Schema<CreateRoom> SCHEMA = new CreateRoomSchema();

    public static CreateRoom getDefaultInstance() { return DEFAULT_INSTANCE; }
    public static Schema<CreateRoom> getSchema() { return SCHEMA; }

    public static final int FIELD_NONE = 0;
    public static final int FIELD_ID = 1;
    public static final int FIELD_TABLE = 2;
    public static final int FIELD_VARIATION = 3;
    public static final int FIELD_STAKE = 4;

    public CreateRoomSchema() {}


    public CreateRoom newMessage() {
        return new CreateRoom();
    }

    public Class<CreateRoom> typeClass() {
        return CreateRoom.class;
    }

    public String messageName() {
        return CreateRoom.class.getSimpleName();
    }

    public String messageFullName() {
        return CreateRoom.class.getName();
    }


    public boolean isInitialized(CreateRoom message) {
        return true;
    }


    public void mergeFrom(Input input, CreateRoom message) throws IOException {
        for (int fieldIx = input.readFieldNumber(this); fieldIx != FIELD_NONE; fieldIx = input.readFieldNumber(this)) {
            mergeFrom(input, message, fieldIx);
        }
    }

    public void mergeFrom(Input input, CreateRoom message, int fieldIx) throws IOException {
        switch (fieldIx) {
            case FIELD_NONE:
                return;
            case FIELD_ID:
                message.setId(input.readString());
                break;
            case FIELD_TABLE:
                message.setTable(input.mergeObject(message.getTable(), de.pokerno.protocol.wire.TableSchema.getSchema()));
                break;

            case FIELD_VARIATION:
                message.setVariation(input.mergeObject(message.getVariation(), de.pokerno.protocol.wire.VariationSchema.getSchema()));
                break;

            case FIELD_STAKE:
                message.setStake(input.mergeObject(message.getStake(), de.pokerno.protocol.wire.StakeSchema.getSchema()));
                break;

            default:
                input.handleUnknownField(fieldIx, this);
        }
    }


    private static int[] FIELDS_TO_WRITE = { FIELD_ID, FIELD_TABLE, FIELD_VARIATION, FIELD_STAKE };

    public int[] getWriteFields() { return FIELDS_TO_WRITE; }

    public void writeTo(Output output, CreateRoom message) throws IOException {
        int[] toWrite = getWriteFields();
        for (int i = 0; i < toWrite.length; i++) {
            writeTo(output, message, toWrite[i]);
        }
    }

    public void writeTo(Output output, CreateRoom message, int fieldIx) throws IOException {
        switch (fieldIx) {
            case FIELD_NONE:
                break;
            case FIELD_ID:
                output.writeString(FIELD_ID, message.getId(), false);
                break;
            case FIELD_TABLE:
                output.writeObject(FIELD_TABLE, message.getTable(), de.pokerno.protocol.wire.TableSchema.getSchema(), false);

                break;
            case FIELD_VARIATION:
                output.writeObject(FIELD_VARIATION, message.getVariation(), de.pokerno.protocol.wire.VariationSchema.getSchema(), false);

                break;
            case FIELD_STAKE:
                output.writeObject(FIELD_STAKE, message.getStake(), de.pokerno.protocol.wire.StakeSchema.getSchema(), false);

                break;
            default:
                break;
        }
    }

    public String getFieldName(int number) {
        switch(number) {
            case FIELD_ID: return "id";
            case FIELD_TABLE: return "table";
            case FIELD_VARIATION: return "variation";
            case FIELD_STAKE: return "stake";
            default: return null;
        }
    }

    public int getFieldNumber(String name) {
        final Integer number = fieldMap.get(name);
        return number == null ? 0 : number.intValue();
    }

    final java.util.Map<String, Integer> fieldMap = new java.util.HashMap<String, Integer>(); {
        fieldMap.put("id", FIELD_ID);
        fieldMap.put("table", FIELD_TABLE);
        fieldMap.put("variation", FIELD_VARIATION);
        fieldMap.put("stake", FIELD_STAKE);
    }
}
